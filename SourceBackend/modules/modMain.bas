...
On Error GoTo eh:
    ...
    qd.sql = "SELECT salesperson_key_id, master_division_cd + RTRIM(master_salesperson_cd) AS report_id_txt"
...
On Error GoTo eh:
    ...
    qd.sql = "SELECT salesperson_key_id, RTRIM(hsi_salesperson_cd) AS report_id_txt"
    qd.sql = qd.sql & " FROM comm_salesperson_master WHERE (select_ind = 1) and comm_plan_id Like 'ESS%' ORDER BY salesperson_key_id"
    
'    qd.SQL = "SELECT salesperson_key_id, master_division_cd + RTRIM(master_salesperson_cd) AS report_id_txt"
'    qd.SQL = qd.SQL & " FROM comm_salesperson_master WHERE (select_ind = 1) ORDER BY salesperson_key_id"
...
Private Function ExportDocuments(sSalespersonKeyId As String, sReportId As String) As Boolean
    ...
    bSuccess = False
    ' rptCommFSCSummary
'    If (ExportReport(sSalespersonKeyId, "", sReportId, "rptCommFSCSummary", "CommissionSummary")) Then
        If (ExportReport(sSalespersonKeyId, "", sReportId, "rptCommStatementDetailReport", "CommissionDetail")) Then
'        If (ExportReport(sSalespersonKeyId, "", sReportId, "rptCommFSCDetail", "CommissionDetail")) Then
...
Private Function ExportDocumentsESS(sSalespersonKeyId As String, sReportId As String) As Boolean
    ...
    If (ExportReport(sSalespersonKeyId, "", sReportId, "rptCommESSDetail", "CommissionDetail")) Then
...
'   Export FSC reports
'''''''''''''''''''''''''''''''''''''''''''''''
' DO NOT RUN - IN SEPARATE DP, tmc 15 Apr 10
'''''''''''''''''''''''''''''''''''''''''''''''

Sub ExportFSCCommBatch()
'    InitParams
'    UpdateSummary
